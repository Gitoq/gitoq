...
// ? gitignore file
export const dispatchGitignore = (values: string[]) => {
  const gitignorePath = path.join(process.cwd(), ".gitignore");

  // Read the .gitignore file content
  let gitignoreContent = "";
  if (fs.existsSync(gitignorePath)) gitignoreContent = fs.readFileSync(gitignorePath, "utf-8");

  // Split the content into lines and trim each line for easier comparison
  const gitignoreLines = gitignoreContent.split("\n").map((line) => line.trim());

  // Filter the entries that are missing in the .gitignore file
  const missingEntries = values.filter((entry) => !gitignoreLines.includes(entry));

  if (missingEntries.length) {
    // If there are missing entries, append them to the .gitignore file
    const newEntries = "# environment variable files (generated by gitoq)" + "\n" + missingEntries.join("\n");
    fs.appendFileSync(gitignorePath, newEntries, { encoding: "utf-8" });
  }
};
